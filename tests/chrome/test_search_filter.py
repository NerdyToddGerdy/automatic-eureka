...
class TestSearchAndFiltering:
    ...
    def test_search_by_filename(self, chrome_driver, base_url, populated_test_db):
        ...
        assert "5 tokens" in main_page.get_token_count()
        ...
        # API verification
        response = requests.get(f"{base_url}/api/tokens?search=goblin")
        assert response.status_code == 200
        assert len(response.json()) == 1

    def test_filter_by_image_type(self, chrome_driver, base_url, populated_test_db):
        ...
        assert "3 tokens" in main_page.get_token_count()

        # API verification
        response = requests.get(f"{base_url}/api/tokens?image_type=Token")
        assert response.status_code == 200
    ...
    def test_filter_by_species(self, chrome_driver, base_url, populated_test_db):
        ...
        response = requests.get(f"{base_url}/api/tokens?species=Goblin")
        assert response.status_code == 200
        tokens = response.json()
        assert len(tokens) == 1
        assert tokens[0]['species'] == 'Goblin'

    def test_combined_filters(self, chrome_driver, base_url, populated_test_db):
        ...
        # API verification
        response = requests.get(f"{base_url}/api/tokens?image_type=Token&search=elf")
        assert response.status_code == 200
        assert len(response.json()) == 1

    def test_clear_filters(self, chrome_driver, base_url, populated_test_db):
    ...
    def test_sort_by_name(self, chrome_driver, base_url, populated_test_db):
        ...
        response = requests.get(f"{base_url}/api/tokens?sort_by=filename&sort_order=ASC")
        assert response.status_code == 200
        tokens = response.json()
        assert len(tokens) == 5
    ...
    def test_sort_by_date(self, chrome_driver, base_url, populated_test_db):
        ...
        # API verification
        response = requests.get(f"{base_url}/api/tokens?sort_by=date_added")
        assert response.status_code == 200
